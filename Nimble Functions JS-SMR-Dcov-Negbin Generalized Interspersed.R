...
dNBVector <- nimbleFunction(
  run = function(x = double(1), mu = double(1), K2D = double(1), theta.d = double(0),
                 z = double(0), z.super = double(0), log = integer(0)) {
    returnType(double(0))
    logProb <- 0
    J <- nimDim(mu)[1]
    if(z.super*z==0){
      ...
      for(j in 1:J){
        if(K2D[j]==1){
          p <- theta.d/(theta.d+mu[j])
          logProb <- logProb + dnbinom(x[j], prob=p, size=theta.d, log=TRUE)
        }else{
          if(x[j]>0){
            return(-Inf)
          }
...
rNBVector <- nimbleFunction(
  run = function(n = integer(0), mu = double(1), K2D = double(1), theta.d = double(0),
                 z = double(0), z.super = double(0)) {
    returnType(double(1))
    J <- nimDim(mu)[1]
      ...
      if(n.samples[gg] > 0){
            ...
            for(k in 1:Kg){
              if(z[i,gg]==1){
                if(model$K2D.sight[gg,j,k]==0){
                  ...
                  ll.y[i,j,k] <- dnbinom(y.sight[i,gg,j,k],prob=p,size=model$theta.d[g],log=TRUE)
                }
                if(y.sight[i,gg,j,k]>0){
        ...
        for(l in 1:n.samples[gg]){
          ...
          if(ID.cand[gg,l] != ID.curr[gg,l]){
            ...
            #update NB likelihoods for the two affected individuals at this detector/year/occasion
            if(model$K2D.sight[gg,j.curr,k.curr]==0){
              if(y.sight.cand[swapped[1],gg,j.curr,k.curr]>0){
                ll.y.cand[swapped[1],j.curr,k.curr] <- -Inf
              }else{
                ll.y.cand[swapped[1],j.curr,k.curr] <- 0
              }
              if(y.sight.cand[swapped[2],gg,j.curr,k.curr]>0){
                ll.y.cand[swapped[2],j.curr,k.curr] <- -Inf
              }else{
                ll.y.cand[swapped[2],j.curr,k.curr] <- 0
              }
            }else{
              p <- model$theta.d[g]/(model$theta.d[g]+model$lam[swapped[1],gg,j.curr])
              ll.y.cand[swapped[1],j.curr,k.curr] <- dnbinom(y.sight.cand[swapped[1],gg,j.curr,k.curr],
                                                             prob=p,size=model$theta.d[g],log=TRUE)
              
              p <- model$theta.d[g]/(model$theta.d[g]+model$lam[swapped[2],gg,j.curr])
              ll.y.cand[swapped[2],j.curr,k.curr] <- dnbinom(y.sight.cand[swapped[2],gg,j.curr,k.curr],
                                                             prob=p,size=model$theta.d[g],log=TRUE)
            }
  ...
  run = function(){
    ...
    for(i in 1:M){
      if(model$z.super[i]==1 & sum(y.req[i,])>0 & y.req[i,1]==0){
        ...
        i.idx.lam <- seq(i,M*n.sight.years,M) #used to reference correct lam nodes
        i.idx.sight <- seq(i,M*n.sight.years*max(K.sight),M) #used to reference correct y.sight nodes
        ...
        for(g in 1:first.det){ #must be recruited in year with first detection or before
          ...
          model$calculate(lam.nodes[i.idx.lam]) #update lam nodes when a z changes
        ...
        if(model$z.start[i]!=z.start.prop){#if proposal is same as current, no need to replace anything
          ...
          model$calculate(ER.nodes) #update ER when N updated
          model$calculate(pd.nodes[i.idx.mark]) #update pd nodes when a z changes
          model$calculate(lam.nodes[i.idx.lam]) #update lam nodes
    ...
    # Detected guys are dynamically updated, these updates happen if sum(y.req[i,]>0)
    for(i in 1:M){
      if(model$z.super[i]==1 & sum(y.req[i,])>0 & y.req[i,n.year]==0){
        z.curr <- model$z[i,]
    ...
    for(i in (n.cap.all+1):M){
      if(model$z.super[i]==1 & sum(y.req[i,])==0){
        ...
        model$calculate(ER.nodes) #update ER when N updated
        model$calculate(pd.nodes[i.idx.mark]) #update pd nodes when a z changes
        model$calculate(lam.nodes[i.idx.lam]) #update lam nodes after z changes
    ...
    for(up in 1:z.super.ups){ #how many updates per iteration?
      ...
      if(updown==0){#subtract
        ...
        if(!reject){
          ...
          pick.idx.lam <- seq(pick,M*n.sight.years,M)
          pick.idx.sight <- seq(pick,M*n.sight.years*max(K.sight),M)
          ...
          model$calculate(lam.nodes[pick.idx.lam]) #turn lam off
        ...
        if(model$N.super[1] < M){ #cannot update if z.super maxed out. Need to raise M
          ...
          model$calculate(lam.nodes[pick.idx.lam]) #turn lam on

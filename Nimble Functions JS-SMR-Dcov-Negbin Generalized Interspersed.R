...
dNBVector <- nimbleFunction(
  run = function(x = double(1), mu = double(1), theta.d = double(0), K2D = double(1), z = double(0), z.super = double(0),
                 log = integer(0)) {
    returnType(double(0))
    if(z.super*z==0){
      if(sum(x)>0){
      ...
      logProb <- sum(dnbinom(x,prob=p,size=theta.d*K2D,log=TRUE))
      return(logProb)
    }
  }
)
...
rNBVector <- nimbleFunction(
  run = function(n = integer(0), mu = double(1), theta.d = double(0), K2D = double(1), z = double(0), z.super = double(0)) {
    returnType(double(1))
    J <- nimDim(mu)[1]
      ...
      if(n.samples[gg] > 0){
          ...
          for(j in 1:Jg){
            for(k in 1:Kg){
              if(z[i,gg]==1){
                ...
                ll.y[i,j,k] <- dnbinom(y.sight[i,gg,j,k],prob=p,
                                       size=model$theta.d[g]*model$K2D.sight[gg,j,k],log=TRUE)
        ...
        for(l in 1:n.samples[gg]){
          ...
          if(ID.cand[gg,l] != ID.curr[gg,l]){
            ...
            #update Negbin likelihoods for the two affected individuals at this detector/primary occasion/occasion
            ...
            ll.y.cand[swapped[1],j.curr,k.curr] <- dnbinom(y.sight.cand[swapped[1],gg,j.curr,k.curr],
                                                           prob=p,size=model$theta.d[g]*model$K2D.sight[gg,j.curr,k.curr],log=TRUE)

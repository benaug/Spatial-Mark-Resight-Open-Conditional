...
nimbuild <- init.SMR.Dcov.Open.Generalized(data,inits,M=M,obsmod="negbin")
...
parameters <- c('N','gamma.fixed','N.recruit','N.survive','N.super','lambda.y1',
                'phi.fixed','p0','lam0','sigma.fixed','theta.d.fixed',
                'theta.marked','theta.unmarked',
                'D0','D.beta1','n.cap')
nt <- 1 #thinning rate
parameters2 <- "ID"
nt2 <- 5
...
conf$addSampler(target=paste0("y.sight[1:",M,",1:",n.year,",1:",max(J.sight),"]"),
                type='IDSamplerOpen',
                control=list(M=M,J.sight=J.sight,n.year=n.year,
...
y.sight.nodes <- Rmodel$expandNodeNames(paste0("y.sight[1:",M,",1:",n.year,",1:",max(J.sight),"]"))
...
for(i in 1:M){
  ...
  conf$addSampler(target = paste("s[",i,", 1:2]", sep=""),
                  type = 'sSamplerDcov',control=list(i=i,res=res,n.cells.x=n.cells.x,n.cells.y=n.cells.y,
                                                     ...
                                                     n.year=n.year,loc.nodes=loc.nodes),
                  silent = TRUE)
}

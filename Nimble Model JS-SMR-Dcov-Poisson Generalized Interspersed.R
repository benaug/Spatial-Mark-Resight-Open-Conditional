NimModel <- nimbleCode({
  ...
  #without individual or primay occasion effects, use for loop to plug into phi[i,g]
  phi.fixed ~ dunif(0,1)
  for(i in 1:M){
    for(g in 1:(n.primary-1)){ #plugging same individual phi's into each primay occasion for custom update
      phi[i,g] <- phi.fixed
    }
  ...
  ##Observation Model##
  #sample type observation model priors (Dirichlet), fixed across primay occasions
  alpha.marked[1] <- 1
  alpha.marked[2] <- 1
  alpha.marked[3] <- 1
  alpha.unmarked[1] <- 1
  ...
  for(g in 1:n.primary){ #sigma informed by data except in primay occasions with no capture effort and no telemetry
    # sigma[g] ~ dunif(0,10) #sigma varies by primay occasion, shared across methods
    sigma[g] <- sigma.fixed #sigma fixed across primay occasions, shared across methods
  }
  #Marking process
  for(g in 1:n.mark.g){
    p0[g] ~ dunif(0,1)
  ...
  for(g in 1:n.sight.g){
    ...
    for(i in 1:M){
      ...
      for(k in 1:K.sight[sight.g[g]]){
        y.sight[i,sight.g[g],1:J.sight[sight.g[g]],k] ~ dPoissonVector(
          lam[i,sight.g[g],1:J.sight[sight.g[g]]] *
            K2D.sight[sight.g[g],1:J.sight[sight.g[g]],k],
          z=z[i,sight.g[g]],
          z.super=z.super[i])
        
        y.event[i,sight.g[g],1:J.sight[sight.g[g]],k,1:3] ~ dmultiOpen(
          y.sight=y.sight[i,sight.g[g],1:J.sight[sight.g[g]],k],
          mark.states=mark.states[i,sight.g[g],k],

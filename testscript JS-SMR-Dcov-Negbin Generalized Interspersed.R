...
theta.d <- rep(0.25,n.primary) #sighting process dispersion parameter, Negbin converges to Poisson as theta.d -> infty
...
inits <- list(p0=rep(0.1,n.primary),lam0=rep(0.25,n.primary),
              sigma=rep(0.5,n.primary),theta.d=rep(1,n.primary))
...
for(g in 1:n.primary){
  ...
  for(i in 1:n.marked[g]){
    ...
    traps <- matrix(numeric(0),nrow=0,ncol=2)
    if(J.sight[g]>0){
      which(rowSums(nimbuild$y.mID[nimbuild$ID.marked[i,g],g,,])>0)
      if(length(trapcaps)>0){
...
for(i in 1:M){
  ...
  conf$addSampler(target = paste("s[",i,", 1:2]", sep=""),
                  type = 'sSamplerDcov',control=list(i=i,res=res,n.cells.x=n.cells.x,n.cells.y=n.cells.y,
                                                     ...
                                                     K.sight=K.sight,n.primary=n.primary,loc.nodes=loc.nodes),
                  silent = TRUE)
}

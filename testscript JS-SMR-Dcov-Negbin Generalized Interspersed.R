...
source("Nimble Model JS-SMR-Dcov-Negbin Generalized Interspersed.R")
source("Nimble Functions JS-SMR-Dcov-Negbin Generalized Interspersed.R") #contains custom distributions and updates
source("sSampler Dcov Open Conditional Generalized Interspersed.R") # activity center sampler that proposes from prior when z.super=0.
source("mask.check.R")
#must run this line 
...
theta.d <- rep(0.25,n.year) #sighting process dispersion parameter, Negbin converges to Poisson as theta.d -> infty
...
obsmod <- "negbin" #must be negbin for this testscript. 
...
#initializing with 1 parameter per session, just set all to same value
inits <- list(p0=rep(0.1,n.year),lam0=rep(0.25,n.year),
              sigma=rep(0.5,n.year),theta.d=rep(1,n.year))
#This function structures the simulated data to fit the model in Nimble (some more restructing below)
nimbuild <- init.SMR.Dcov.Open.Generalized.Interspersed(data,inits,M=M,obsmod="negbin")
...
Niminits <- list(N=nimbuild$N,N.survive=nimbuild$N.survive,N.recruit=nimbuild$N.recruit,
                 ...
                 sigma.fixed=inits$sigma[1],theta.d.fixed=inits$theta.d[1],
...
parameters <- c('N','gamma.fixed','N.recruit','N.survive','N.super','lambda.y1',
                'phi.fixed','p0','lam0','theta.d.fixed','sigma.fixed','theta.marked','theta.unmarked',
                'D0','D.beta1','n.cap')
nt <- 1 #thinning rate
parameters2 <- "ID"
nt2 <- 5
...
conf$addSampler(target=Rmodel$expandNodeNames(paste("y.sight[1:",M,",1:",n.year,",1:",max(J.sight),",1:",max(K.sight),"]")),
                type='IDSamplerOpen',
                control=list(M=M,J.sight=J.sight,K.sight=K.sight,n.year=n.year,
...
y.sight.nodes <- Rmodel$expandNodeNames(paste("y.sight[1:",M,",1:",n.year,",1:",max(J.sight),",1:",max(K.sight),"]"))
...
for(i in 1:M){
  ...
  conf$addSampler(target = paste("s[",i,", 1:2]", sep=""),
                  type = 'sSamplerDcov',control=list(i=i,res=res,n.cells.x=n.cells.x,n.cells.y=n.cells.y,
                                                     ...
                                                     K.sight=K.sight,n.year=n.year,loc.nodes=loc.nodes),
                  silent = TRUE)
}
